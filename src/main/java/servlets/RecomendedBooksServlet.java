...
public class RecomendedBooksServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        List<Book> recomendendBooksList = dataProvider.getRecomendedBooks(editionName);
        pw.println("<html>");
        pw.println("<head>");
        ...
        pw.println("<h2><center> " + "Recomandari" + "</center>");
        pw.println("</h2>");
        pw.println("<hr>");
        pw.println("</div>");
        for (int i = 0; i < recomendendBooksList.size(); i++) {
            int bookId = recomendendBooksList.get(i).getId();
            String img = "<a href=\"book?id=" + bookId +
                    "\"><img height=\"300\" width=\"200\" class='img-rounded' src='images/Carti/"
                    + recomendendBooksList.get(i).getImage() + "'/></a>";
            pw.println(img);
        }
        pw.println("<hr/>TOTAL: " + recomendendBooksList.size() + " carti.");
